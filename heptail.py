...
def hephead(inputfile,outputfile,n_events):
    ...
    n_events = tot - n_events if n_events < tot else 0 


    o1 = hm.WriterAscii(outputfile)
    for i in range(tot - n_events):
        temp_compose = EventComposer(e1.findParticularEvent(n_events+i))
